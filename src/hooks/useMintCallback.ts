...
import { useMintState } from 'state/trade/reducer'
...
export default function useMintCallback(
  ...
  const pendingMint = useHasPendingMint(addressMap)
  ...
  const minimumAmountOutBN: BigNumber = useMemo(() => {
    ...
    const slippedAmountOut = new BN(amountOut).times(slippageFactor).toString()
